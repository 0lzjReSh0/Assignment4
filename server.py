...
def execute_client(client_socket,port):
    ...
    all_rooms = ", ".join(chatrooms.keys())
    client_socket.send(f"All available chat rooms here: {all_rooms}".encode('utf-8'))
    ...
    try:
        while True:
            ...
            if message.startswith("#quit"):
                all_rooms = ", ".join(chatrooms.keys())
                client_socket.send(f"Available Chat Room: {all_rooms}".encode('utf-8'))
